...
\abstract{Split federated learning (SplitFed) pairs the label locality of split learning with federated aggregation, but existing pipelines assume synchronous aggregation and benign clients. We propose SENTINEL-SplitFed, a server-side extension that replaces blocking aggregation with timed submission windows, tracks per-client staleness, and gates each submission by a Staleness-Normalized Anomaly Score (SNAS) combining cosine and norm encoder anomalies discounted by benign delay-induced staleness. On a three-client non-IID MIMIC-IV mortality benchmark, SNAS detects gradient-scaling, free-rider, label-flip and backdoor attacks (the last a sparse weight corruption, not a trigger-based backdoor) in 100\% of post-warm-up attacker submissions (8/8, Wilson 95\% CI $[0.68,1.00]$) at no observed false positives (0/16, CI $[0.00,0.19]$), and does not falsely quarantine honest stragglers. Detection is not exclusion: direction-driven attacks score between the flag and quarantine thresholds, so the backdoor arm is flagged every round yet still inflicts the largest degradation measured ($0.7008$ AUROC against a $0.9651$ clean baseline). Two headline comparisons do not survive a valid configuration, and we report both. At $N=3$ SNAS beats trimmed mean and FLTrust on every attack setting, ten of ten paired seeds (Wilcoxon $p=0.00195$); repartitioning into six clients, where Krum's $N\geq2f+3$ condition holds and the trimmed mean stops discarding zero values, shows this to be a property of those baselines rather than of SNAS --- trimmed mean gains $0.134$ AUROC on the free-rider attack and overtakes it, placing SNAS fourth of five. Krum's apparent $N=3$ advantage is likewise a tie-break artifact that disappears at $N=6$. SNAS's advantage is confined to the setting it targets, a client straggling from a small federation, where quorum-dependent robustness degrades while per-submission detection does not; that setting is evaluated at $N=3$ only. Adaptive experiments identify a further boundary: an attack-then-abstain adversary evades memoryless staleness normalization, which we show to be self-limiting.}
...
\begin{itemize}
    \item \textbf{Gradient scaling:} every floating-point encoder tensor is multiplied by a scalar $s$ (default $s=10$), preserving direction exactly while inflating magnitude.
    \item \textbf{Free rider:} every floating-point tensor is replaced by i.i.d.\ samples from $\mathcal{N}(0,1)$, so the submission carries no learned signal.
    \item \textbf{Label-flip proxy:} i.i.d.\ Gaussian noise with $\sigma=0.3$ is added to every floating-point tensor.
    \item \textbf{Backdoor:} a constant $+5$ is added to a uniformly random $10\%$ of floating-point weights, leaving the remainder untouched.
...
\paragraph{Naming, stated precisely.} Every attack above is a perturbation applied in \emph{weight space} to the encoder state a client submits for aggregation; none of them modifies the client's local data, labels, or training procedure. Two of the labels are inherited from the configuration files and are looser than the terms used in the poisoning literature, so we state plainly what they do and do not mean.

The \textbf{backdoor} arm is a sparse, high-magnitude weight corruption. It is \emph{not} a trigger-based backdoor in the sense of BadNets or SafeSplit~\cite{safesplit2025}: there is no input trigger, no attacker-chosen target class, and no attempt to preserve clean accuracy --- indeed its defining empirical feature here is that clean accuracy collapses (Section~\ref{sec:results}), which is the opposite of the stealth a true backdoor seeks. It should be read as a sparse-corruption stress test of the detector's direction term, and results reported under this name carry no implication about trigger-based backdoor robustness, which we do not evaluate. Similarly, \textbf{free rider} in the federated-learning literature often denotes a client that resubmits the unchanged global model to harvest the benefits of federation without doing work; the variant implemented here submits \emph{random} weights, which is more disruptive and is elsewhere called a random or Gaussian attack. The \textbf{label-flip proxy} is named a proxy because it perturbs the encoder in the direction a label-flipped local run would, without flipping any labels. We retain all three names for continuity with the released configurations and logs, and analyse them by what they actually do.
...
\paragraph{A third signal, implemented and disabled.} The score above uses two weight-space terms. Our implementation also computes a third, an \emph{activation-divergence} term $A^{\mathrm{act}}_i$ --- the Gaussian-approximation symmetric KL divergence between a client's current activation batch and a rolling exponential-moving-average reference for that client --- entering as $\mathrm{SNAS}_i = (\alpha A^{\mathrm{act}}_i + \beta A^{\mathrm{cos}}_i + \gamma A^{\mathrm{norm}}_i)/(1+\tau_i)$. It is the one signal that is genuinely specific to split learning, since only SplitFed routes client activations through the server as a matter of protocol. We set $\alpha=0$ in every experiment reported here, and say so explicitly rather than presenting the two-term score as the whole design.

The reason is mechanical. In our threat model the adversary perturbs only the encoder state it \emph{submits} for aggregation; it trains honestly through the split forward/backward exchange (Section~\ref{sec:problem}). Its activations are therefore those of an honestly trained client and carry no anomaly to detect, so $A^{\mathrm{act}}$ is non-discriminative against every attack evaluated here, and including it would only dilute the two terms that do separate. This bounds a claim that would otherwise be too strong: with $\alpha=0$, the operative detector is cosine plus norm on submitted weights, which is computable in \emph{any} federated setting and is not split-learning-native. What is specific to SplitFed in this work is the aggregation object --- the client-side encoder rather than a full model --- together with the staleness semantics of windowed submission, not the use of activations. An adversary that poisons during training rather than at submission would move activations and is the setting in which $\alpha>0$ should earn its place; evaluating it requires the server-head threat model we place out of scope in Section~\ref{sec:limitations}.
...
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{2.15cm}>{\raggedright\arraybackslash}p{2.35cm}ccc>{\raggedright\arraybackslash}X@{}}
\toprule
...
Temporal detectors
& Detect inconsistent update paths
& Limited
& Yes
& No
...
& Intrusion-resilient async SplitFed
& \textbf{Yes}
& \textbf{Yes}
& \textbf{Yes}
...
\paragraph{Interval estimates for the security rates.} DR and FPR are proportions estimated from a small number of gate decisions --- eight scored attacker submissions and sixteen honest client-rounds per experiment --- and both land on the boundary of the unit interval, where the normal approximation is degenerate and reports a zero-width interval. We therefore accompany every DR and FPR with a $95\%$ \emph{Wilson score} interval, which remains defined at $\hat{p}=0$ and $\hat{p}=1$. This matters for how the headline numbers should be read: a detection rate of $8/8$ is a point estimate of $100\%$ with interval $[0.68, 1.00]$, and a false-positive rate of $0/16$ is $0\%$ with interval $[0.00, 0.19]$. The point estimates are what the runs produced; the intervals are what a sample of this size can support, and we quote both rather than presenting $100\%$ and $0\%$ as though they were tightly estimated. Widening these intervals requires more scored rounds per run, not more seeds, since the gate decisions within a run are the sampling unit.
...
\paragraph{Multiple comparisons.} The paper reports many paired tests --- twelve SNAS-versus-baseline comparisons (four aggregators $\times$ three attack settings), four gate-ablation comparisons, and twelve factorial contrasts --- so the per-test $\alpha=0.05$ is not by itself an adequate error rate, and we state how the results stand under correction rather than leaving it to the reader. Treating the aggregator comparisons as one family, Bonferroni gives $\alpha=0.05/12=0.0042$, above the attained Wilcoxon floor of $0.00195$; the ablation family of sixteen contrasts gives $\alpha=0.0031$, also above it. Every comparison therefore survives family-wise correction on the nonparametric test. Pooling all twenty-eight tests into a single family would give $\alpha=0.00179$, marginally \emph{below} the $n=10$ Wilcoxon floor --- so under that most conservative accounting the nonparametric test cannot certify the results, not because the effects are weak but because ten seeds cannot produce a smaller $p$ than $0.00195$ however large the effect. The paired $t$-tests are not subject to that floor and settle the question: the largest $p$ among the twelve attack comparisons is $2.7\times10^{-4}$, which clears Bonferroni correction over all twenty-eight tests, and over forty, by more than an order of magnitude. We report both tests throughout for this reason. The one comparison we do \emph{not} claim as significant is the honest-straggler null (Wilcoxon $p=0.32$), which is intended to be null.
...
\paragraph{Two further attack types, and the limit of a single quarantine threshold.} Exp~5 and Exp~6 exercise the two attacks that perturb the \emph{direction} of the submitted encoder rather than its magnitude: a label-flip proxy adding i.i.d.\ Gaussian noise, and a backdoor perturbing a random $10\%$ of weights by $+5$ (Section~\ref{sec:problem} states precisely what these names do and do not imply; the backdoor arm is a sparse weight corruption, not a trigger-based backdoor). Both are detected in $100\%$ of post-warm-up attacker rounds with no false positives, so the gate sees them as reliably as it sees gradient scaling.
...
\caption{Aggregator comparison across Exp~1, Exp~2, and Exp~3. All bars are ten-seed means. Krum attains the highest raw AUROC but does so by selecting Client 0 in every aggregation round (300 of 300 selection decisions, zero exceptions) — a tie-break artifact of $N=3$ falling outside Krum's $N\geq2f+3$ validity condition, not a robustness property (Section~\ref{sec:n6}) — discarding the Surgical and Cardiac partitions. Exp~2 (free rider) is the setting that separates the aggregators: SNAS leads FLTrust by $+0.033$ and trimmed mean by $+0.085$ AUROC, each on ten of ten paired seeds. \textbf{The trimmed-mean margin does not survive a valid configuration}: at $N=6$ trimmed mean gains $0.134$ AUROC and overtakes SNAS, and the lead shown here is retracted at $N\geq6$ (Section~\ref{sec:n6}).}
\label{fig:baselines}
\end{figure}

\begin{table}[htbp]
\caption{Baseline comparison. \textbf{All values are ten-seed means}, reported as AUROC~/~AUPRC per cell. Krum reaches higher raw AUROC but collapses federation by repeatedly selecting one client. The AUPRC column is the more discriminating one: on the free-rider attack (Exp~2) SNAS leads trimmed mean by $+0.085$ AUROC but by $+0.248$ AUPRC, and FLTrust by $+0.033$ AUROC but $+0.118$ AUPRC, so AUROC materially understates the separation between the aggregators at this prevalence. \textbf{The trimmed-mean column must be read against Section~\ref{sec:n6}}: at $N=3$ the rule trims zero values and is plain averaging, and once validly configured at $N=6$ it overtakes SNAS on this attack. The lead over trimmed mean shown here is a property of that degenerate configuration and is retracted at $N\geq6$.}
\label{tab:baselines}
\centering
...
On raw accuracy the five aggregators are separated by $0.003$ AUROC at $N=6$, with the two order statistics ahead of SNAS by roughly $0.001$; we therefore claim no accuracy advantage for SNAS in this configuration. We deliberately report no significance test here. This grid uses five seeds rather than the ten used throughout Section~\ref{sec:results}, and by the argument of Section~\ref{sec:setup} a five-seed paired Wilcoxon cannot reach conventional significance whatever the effect size, so a $p$-value would be uninformative rather than merely weak. The claim this section does make --- that Krum's single-client collapse is specific to $N=3$ --- rests on a count over $50$ selection decisions rather than on a comparison of means, and is unaffected by the seed budget. Gradient scaling is a magnitude attack that norm clipping, order statistics and reference-gradient weighting all address by construction, so it is the attack least able to separate correctly configured defenses --- consistent with Exp~1 being the narrowest margin in Table~\ref{tab:baselines} as well. What the $N=6$ grid establishes here is narrower and was the reason for running it: the federation-collapse behavior attributed to Krum in Table~\ref{tab:entropy} is specific to the under-specified $N=3$ regime, and both Krum and trimmed mean are measured at their intended operating point. The separation reported for SNAS arises instead on the free-rider attack (Exp~2), where it leads trimmed mean by $+0.085$ and FLTrust by $+0.033$ AUROC at $N=3$. That is the largest claim in this paper and the one most exposed to the configuration problem just described, so we ran it at $N=6$ as well.

\paragraph{The free-rider attack at $N=6$, and what it retracts.} Table~\ref{tab:n6-e2} reports Exp~2 across all five aggregators at $N=6$, and the outcome does not favour our method. Trimmed mean rises from $0.8242$ to $0.9578$ --- a gain of $+0.134$ AUROC obtained purely by being validly configured --- and overtakes SNAS at $0.9313$. The coordinate median and Krum land at $0.9579$ and $0.9567$, also ahead of SNAS, which places SNAS fourth of five. The mechanism is the same arithmetic as before: at $N=3$ and trim ratio $0.2$ the rule discards $\lfloor0.6\rfloor=0$ values and is plain averaging, so the free rider's random weights enter the aggregate at full weight; at $N=6$ it discards one value per tail, which removes the free rider outright.

We therefore retract, at $N\geq6$, the reading that SNAS is more robust to free-riding than trimmed mean. What the $N=3$ comparison measured was a degenerate baseline, not a property of the detector --- the same failure that produced Krum's apparent \emph{advantage} in Table~\ref{tab:n6-auroc}, operating here in the opposite direction. Both are artifacts of the low-client regime and both are corrected by this grid. The claim that survives is the narrower one already stated in Section~\ref{sec:results}: SNAS is not the strongest robust aggregator under full participation, and we do not present it as one.

SNAS's residual deficit has a specific and measurable cause rather than being unexplained. It is incurred during the two warm-up rounds, in which the gate is deliberately inactive while the reference encoder is established (Section~\ref{sec:method}), so the free rider's random weights enter the first two aggregates unopposed and the run does not fully recover within ten rounds. Order statistics carry no warm-up: the median and the trimmed mean reject an outlying submission from round one, because their robustness is a property of the estimator rather than of a reference that must first be learned. This is a genuine architectural disadvantage of reference-based detection at small round budgets, and it is a more useful result than the $N=3$ number it replaces. It also indicates a concrete remedy we have not implemented: seeding the reference from the initialization shared at registration, so that round one is scored rather than skipped.

Two limits bound how far Table~\ref{tab:n6-e2} should be read. First, Exp~2 has no straggler --- every client submits inside the window --- so this is the full-participation regime in which quorum-based rules are strongest and the property SNAS is designed for is not exercised at all; the straggler comparison is evaluated at $N=3$ only (Section~\ref{sec:limitations}). Second, the $N=6$ E1 grid of Table~\ref{tab:n6-auroc} and this E2 grid were executed on different hardware, E1 on a CUDA device and E2 on CPU. Every aggregator within each table is matched on seeds, data and code, so the comparisons inside a table are sound; the two tables should not be compared against each other at the fourth decimal.

\begin{table}[htbp]
\caption{Exp~2 (free rider) at $N=6$, five seeds, against the ten-seed $N=3$ values from Table~\ref{tab:baselines}. This is the attack on which SNAS's separation is largest at $N=3$, which is why it was run. The $N=3$ column is reproduced to make the reversal explicit: trimmed mean gains $+0.134$ AUROC simply by being validly configured, and overtakes SNAS. As in Table~\ref{tab:n6-auroc}, the two columns are not paired and compare regimes rather than matched runs.}
\label{tab:n6-e2}
\centering
\begin{tabular}{lccc}
\toprule
Aggregator & $N=6$ (5 seeds) & $N=3$ (10 seeds) & $\Delta$ \\
\midrule
Coordinate median & $\mathbf{0.9579} \pm 0.0004$ & $0.9647 \pm 0.0004$ & $-0.007$ \\
Trimmed mean      & $0.9578 \pm 0.0004$ & $0.8242 \pm 0.0189$ & $\mathbf{+0.134}$ \\
Krum              & $0.9567 \pm 0.0002$ & $0.9647 \pm 0.0004$ & $-0.008$ \\
SNAS              & $0.9313 \pm 0.0017$ & $0.9094 \pm 0.0070$ & $+0.022$ \\
FLTrust           & $0.8991 \pm 0.0042$ & $0.8762 \pm 0.0134$ & $+0.023$ \\
\bottomrule
\end{tabular}
\end{table}
...
\label{tab:adaptive}
\centering
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{3.2cm}cccc>{\raggedright\arraybackslash}X@{}}
\toprule
Attack & Detection & First det. & Mean SNAS & AUROC $\Delta$ & Interpretation \\
\midrule
Threshold-aware scaling & 100\% & Round 2 & 0.347 & $-0.0040$ & Defeated by training drift \\
...
Alternating, step & 0\% & Never & 0.175 & $-0.0040$ & Worst of the four decays \\
Slow ramp, 15 rounds & 38.5\% & Round 10 & 0.221 & $-0.0009$ & Flagging bounds damage \\
\bottomrule
\end{tabularx}
\end{table}
...
This study has eight main limitations. First, the primary benchmark uses three clients, reflecting a realistic low-client healthcare federation but limiting conclusions about large-scale cross-device FL. This bound is also what places Krum outside its own $N\geq2f+3$ validity condition and reduces the trimmed mean to an unweighted average, so the $N=3$ comparison characterizes both baselines in an under-specified regime rather than at their intended operating point. Section~\ref{sec:n6} corrects both baselines at $N=6$ for the two full-participation attacks, and in doing so \textbf{retracts the free-rider advantage claimed at $N=3$}: validly configured, trimmed mean overtakes SNAS and the method places fourth of five. The straggler settings --- which are the ones SNAS is actually designed for, and on which the paper's defended claim rests --- are evaluated at $N=3$ only. At $N=6$ a single dropout still leaves a five-client quorum, so we expect the median's advantage to return and the gap to narrow; we predict this rather than demonstrate it, and did not run it. The scenario this paper targets, low-client cross-silo healthcare federation, is one in which $N=3$ is a realistic operating point rather than a convenience --- three ICU services, three collaborating hospitals --- but the consequence must be stated plainly: our central claim is \emph{demonstrated at one federation size and predicted, not shown, at others}. Sweeps over the fraction of malicious clients, which require $N\geq5$ to be meaningful at $f\geq2$, are likewise not attempted.
...
We introduced SENTINEL-SplitFed, a server-side framework for asynchronous split federated learning with intrusion resilience in critical-care mortality prediction. The method combines timed aggregation windows, staleness tracking, staleness-normalized anomaly scoring, norm clipping, and gate-based robust aggregation. On non-IID MIMIC-IV clients, SENTINEL-SplitFed preserves clean asynchronous performance, avoids false quarantine of honest stragglers, and detects the evaluated gradient-scaling, free-rider, label-flip and backdoor attacks in 100\% of post-warm-up attacker submissions (8/8, Wilson 95\% CI $[0.68,1.00]$; 80\% including the warm-up rounds) at 0\% observed false positives (0/16 honest client-rounds, CI $[0.00,0.19]$). Detection, however, is not exclusion: the two direction-driven attacks are flagged in every scored round yet score below the quarantine threshold, so they continue to enter the aggregate at reduced weight. Baseline comparisons reveal that standard robust FL methods require care before they transfer to low-client SplitFed. In the three-client regime Krum collapses federation to a single client and the trimmed mean degenerates to an unweighted average; both are consequences of that regime rather than of the methods, and both resolve once the federation is repartitioned into six clients, where Krum discriminates genuinely and never selects the adversary. That correction cuts both ways, and we report it in both directions: at $N=6$ the trimmed mean also stops being degenerate, gains $0.134$ AUROC on the free-rider attack and overtakes SNAS, so the largest advantage claimed at $N=3$ does not survive a valid configuration and is retracted at $N\geq6$. SNAS's advantage is confined to the setting it was designed for --- a client straggling out of a small federation --- and that setting is evaluated at $N=3$ only. FLTrust is compatible with split-learning encoder aggregation, but only once its reference gradient is derived from the previous round's global encoder and trust is computed over updates rather than absolute weights; without both adaptations it silently degenerates to uniform averaging. Adaptive attacks also reveal a clear robustness boundary, motivating temporal SNAS accumulation. Overall, SENTINEL-SplitFed provides a practical and empirically grounded step toward resilient split federated learning for critical healthcare applications.
...
\bmhead{Use of AI-assisted technology} Large language model assistance (Claude, Anthropic) was used during this work for software implementation and debugging, experiment orchestration, analysis scripting, and drafting and editing of the manuscript text. No AI system is an author. All experimental results were produced by the authors' own code, and every numeric claim in this paper was verified by the authors against the committed result artifacts from which it derives. The authors take full responsibility for the content, analysis, and conclusions.

\bmhead{Author contributions} S.T.N. supervised the project and reviewed and edited the manuscript. T.A.R. led the methodology, including the SNAS formulation, staleness modeling, and robust aggregation rule. H.A.N. led the software implementation and experiments, including the MIMIC-IV pipeline, attack and baseline evaluations, and statistical analysis. M.S.R. led the related-work review and manuscript writing. All authors read and approved the final manuscript.
