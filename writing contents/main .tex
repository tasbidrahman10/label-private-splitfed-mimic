...
\affil*[1]{\orgdiv{Department of Computer Science and Engineering}, \orgname{North South University}, \orgaddress{\city{Dhaka}, \country{Bangladesh}}}
...
\abstract{Split federated learning combines the label locality of split learning with the collaborative model aggregation of federated learning, making it attractive for privacy-sensitive critical-care prediction. However, existing SplitFed pipelines commonly assume synchronous aggregation and benign clients: the server waits for every client before aggregation and accepts each submitted encoder without adversarial verification. These assumptions are brittle in Internet of Medical Things (IoMT) deployments, where clients may be delayed by network or workload variation and compromised clients may submit poisoned encoder weights. We propose SENTINEL-SplitFed, a server-side extension for asynchronous split federated learning with intrusion resilience. The framework replaces blocking aggregation with timed asynchronous windows, tracks per-client staleness, and introduces a Staleness-Normalized Anomaly Score (SNAS) that combines cosine and norm-based encoder anomalies while discounting benign delay-induced staleness. Accepted, flagged, and quarantined client submissions are then handled by a robust asynchronous aggregation rule with norm clipping. On a three-client non-IID MIMIC-IV mortality prediction benchmark, SENTINEL-SplitFed preserves clean asynchronous performance relative to synchronous SplitFed, maintains AUROC under honest straggler dropout, and detects gradient-scaling and free-rider attacks with 100\% detection of post-warm-up attacker submissions (8/8; 80\% including the warm-up rounds, during which the gate is deliberately inactive) and 0\% observed false positives (0/16 honest client-rounds) in the evaluated settings. Across ten seeds, SNAS obtains AUROC $0.9613\pm0.0001$ under gradient scaling, $0.9595\pm0.0004$ under simultaneous attack and straggler, and $0.9094\pm0.0070$ under free-rider attack. Against robust-aggregation baselines, SNAS is significantly better than both trimmed mean and FLTrust on every attack setting, winning all ten of ten paired seeds in each case (Wilcoxon $p=0.00195$, the $n=10$ floor). Krum attains higher raw AUROC, but does so by collapsing federated participation to a single client in 300 of 300 aggregation rounds, a tie-break artifact of operating outside its own $N\geq2f+3$ validity condition rather than a robustness property. Adaptive adversary experiments further reveal a structural vulnerability of memoryless staleness normalization: an attack-then-abstain adversary can evade per-round SNAS gates. We discuss temporal accumulation as a mitigation direction. These results position SENTINEL-SplitFed as a split-learning-native resilience mechanism for asynchronous low-client critical-care federation, while explicitly characterizing its robustness boundary.}
...
\begin{itemize}
    ...
    \item We evaluate SENTINEL-SplitFed on non-IID MIMIC-IV mortality prediction using honest straggler, gradient-scaling, free-rider, simultaneous attack-and-straggler, adaptive adversary, and hyperparameter sensitivity experiments.
    \item We compare SNAS against Byzantine-robust FL baselines under matched conditions and show that it significantly outperforms trimmed mean and FLTrust on every attack setting, while Krum's apparent advantage comes from collapsing federation rather than from robustness. We quantify that collapse with a contribution-entropy metric measured on the same runs: $H=0.000$ for Krum, which selects one client in every round, versus $H=0.986$ for SNAS in the clean setting and $H=0.249$--$0.767$ under attack, where the reduction reflects deliberate exclusion of the detected adversary.
...
In summary, the intersection of asynchronous execution and adversarial robustness within split federated learning exposes a critical multi-dimensional research gap. Existing asynchronous SplitFed architectures (such as AFSL and GAS) focus strictly on system efficiency and straggler latency mitigation, completely omitting active defense layers against compromised participants. Conversely, state-of-the-art asynchronous Byzantine defenses (such as AFLGuard and feature fingerprinting frameworks) are architecturally tied to standard, full-model federated learning pipelines; they are functionally incompatible with label-private SplitFed where the central server lacks direct access to raw data or full-network parameters to compute standalone reference gradients. Furthermore, while emerging quantum-enhanced distributed paradigms (QFL and QSL) and localized embedding filters (such as SafeSplit) lay vital blueprints for future secure topographies, they do not yet address the temporal overlap where honest, networking-induced staleness mimics active encoder poisoning anomalies. 

\emph{SENTINEL-SplitFed} explicitly fills this void. It targets an underexplored intersection: an asynchronous, horizontal SplitFed architecture that jointly handles benign stragglers and malicious encoder submissions via a low-overhead, server-side decision rule. Rather than claiming dominance over generalized, massive-scale Byzantine-robust FL, we establish a specialized architectural defense native to low-client, label-private, critical-care split-learning federations. Table~\ref{tab:sota-positioning} summarizes this positioning against representative FL and SplitFed robustness methods.
...
The goal is not to provide cryptographic Byzantine security. Instead, the goal is to design and empirically characterize a split-learning-native detection and aggregation mechanism that resists realistic encoder-submission attacks while avoiding false quarantine of benign stragglers.
...
SENTINEL-SplitFed inserts four modules between the client encoder submission endpoint and the aggregation logic: an asynchronous round controller, a staleness registry, the SNAS detection gate, and robust asynchronous aggregation. The client-side split-learning protocol remains unchanged.
...
The detector uses a one-round warm-up period to establish a reference global encoder. After warm-up, each submitted client receives one of three gate decisions:
\begin{equation}
    ...
    \end{cases}
\end{equation}
The default thresholds are $\delta_{\mathrm{flag}}=0.22$ and $\delta_{\mathrm{quarantine}}=0.35$. Flagged clients are included with an aggregation penalty, while quarantined clients are excluded for that round.
\begin{table}[t]
...
& Yes
& No
& Higher raw AUROC, but repeatedly selects one client, causing federation collapse. \\
...
& Yes
& No
& Degenerates at three clients; with trim ratio 0.2, no values are trimmed. \\
...
& Yes
& No
& Mismatched with SplitFed because the server cannot compute a meaningful trusted encoder gradient. \\
...
& \textbf{Yes}
& \textbf{Yes}
& Preserves federation, detects gradient-scaling and free-rider attacks, avoids false quarantine of honest stragglers, but remains vulnerable to attack-then-abstain alternation. \\
...
\paragraph{Architecture and schedule.} The client encoder maps the 265 input features to a 32-dimensional representation through a hidden layer of size 64. The server top model maps the 32-dimensional activation to a binary mortality prediction through a hidden layer of size 64. Unless otherwise stated, experiments use 200 batches per client per round, batch size 64, and 10 aggregation rounds.
...
Table~\ref{tab:main-results} summarizes the primary SNAS results. To ensure that the reported degradations are measured against a comparably-estimated reference, the clean asynchronous baseline is itself evaluated across the same ten seeds, giving $0.9651\pm0.0001$ AUROC; this matches the synchronous SplitFed baseline to within noise. Gradient scaling and attack-plus-straggler experiments retain AUROC within approximately 0.5--0.7 percentage points of this baseline while detecting the attacker in all evaluated detection rounds. The free-rider attack is more destructive because random encoder weights corrupt the warm-up round, but quarantine after warm-up enables partial recovery. The honest-straggler setting causes no degradation relative to the clean baseline.

\begin{table}[t]
\caption{Primary SENTINEL-SplitFed results. AUROC is averaged over rounds 8--10; the clean baseline and all four experiments report ten-seed mean and standard deviation. Detection (DR) and false positive (FPR) rates are observed under the evaluated experiments. DR is reported over post-warm-up attacker submissions (8/8 detection rounds; first detection in round 2, immediately after warm-up), which is $80\%$ of all $10$ attacker submissions including the two warm-up rounds during which the gate is deliberately inactive. FPR is $0/16$ observed honest client-rounds.}
\label{tab:main-results}
\centering
\begin{tabular}{llllll}
\toprule
Experiment & Attack & Straggler & AUROC & DR & FPR \\
\midrule
Sync-FedAvg baseline & None & None & 0.9649 (1 seed) & -- & -- \\
Async clean (n=10) & None & Mild lag & $0.9651\pm0.0001$ & -- & -- \\
Exp~1 gradient scaling & Client 1, $10\times$ & None & $0.9613\pm0.0001$ & 100\% & 0\% \\
Exp~2 free rider & Client 2, random & None & $0.9094\pm0.0070$ & 100\% & 0\% \\
Exp~3 attack + straggler & Client 1, $10\times$ & Client 2 & $0.9595\pm0.0004$ & 100\% & 0\% \\
Exp~4 honest straggler & None & Client 2 & $0.9659\pm0.0002$ & -- & 0\% \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:perround} shows the per-round dynamics behind these summary numbers. Under gradient scaling (Exp~1) and simultaneous attack-plus-straggler (Exp~3), the global encoder dips in the warm-up round and then recovers to track the baseline once the attacker is quarantined. The free-rider attack (Exp~2) causes a much larger warm-up crash because random encoder weights corrupt the first aggregation before the gate is calibrated, followed by steady recovery after quarantine. The honest-straggler run (Exp~4) tracks the baseline throughout, confirming that staleness normalization prevents false quarantine of the delayed client.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_perround_auroc.pdf}
\caption{Per-round mean AUROC across the three client validation partitions (seed 42). The dashed line is the synchronous SplitFed baseline (0.9649). Exp~1/Exp~3 dip during warm-up and recover; Exp~2 shows the free-rider warm-up crash and subsequent recovery; Exp~4 tracks the baseline without false quarantine.}
\label{fig:perround}
\end{figure}
...
Trimmed mean degenerates to vanilla FedAvg at $N=3$ with trim ratio 0.2 because $\lfloor 0.2\times3\rfloor=0$ coordinates are trimmed; it is therefore reported as a nominal rather than an effective robust baseline at this client count.
...
\caption{Aggregator comparison across Exp~1, Exp~2, and Exp~3. All bars are ten-seed means. Krum attains the highest raw AUROC but does so by selecting Client 0 in every aggregation round (300 of 300 selection decisions, zero exceptions) — a tie-break artifact of $N=3$ falling outside Krum's $N\geq2f+3$ validity condition, not a robustness property — discarding the Surgical and Cardiac partitions. Exp~2 (free rider) is the setting that separates the aggregators: SNAS leads FLTrust by $+0.033$ and trimmed mean by $+0.085$ AUROC, each on ten of ten paired seeds.}
\label{fig:baselines}
\end{figure}

\begin{table}[t]
\caption{Baseline comparison. SNAS and Krum AUROC values are ten-seed means; trimmed mean and FLTrust are single representative runs. Krum reaches higher raw AUROC but collapses federation by repeatedly selecting one client. Trimmed mean and FLTrust degenerate under the evaluated low-client split-learning setting.}
\label{tab:baselines}
\centering
\begin{tabularx}{\textwidth}{@{}lccc>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
Aggregator & Exp~1 AUROC & Exp~2 AUROC & Exp~3 AUROC & Split-learning compatible? & Federation preserved? \\
\midrule
SNAS & $\mathbf{0.9613}$ & $\mathbf{0.9094}$ & $\mathbf{0.9595}$ & Yes & Yes \\
Krum & 0.9646 & 0.9647 & 0.9646 & Partially & No ($H=0$) \\
Trimmed mean & 0.9569 & 0.8242 & 0.9560 & Yes & Nominally \\
FLTrust & 0.9593 & 0.8762 & 0.9583 & Yes & Nominally \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[t]
\caption{Gate ablation. ``No-detection'' is SENTINEL-SplitFed with the anomaly gate and norm clipping disabled (pure staleness-weighted async FedAvg). Gate benefit is the paired AUROC gain from re-enabling the gate; $p$ is a paired $t$-test over ten seeds, pairing runs by seed. The gain is positive on all ten seeds in each of the three attack settings, and on five of ten (i.e.\ at chance) on the attack-free honest-straggler setting.}
\label{tab:ablation}
\centering
\begin{tabular}{lcccc}
\toprule
Experiment & SNAS (gate on) & No-detection & Gate benefit & Paired $p$ \\
\midrule
Exp~1 gradient scaling & $0.9613\pm0.0001$ & $0.9578\pm0.0002$ & $+0.0035$ & $6\times10^{-12}$ \\
Exp~2 free rider & $0.9094\pm0.0070$ & $0.8141\pm0.0137$ & $+0.0953$ & $2\times10^{-8}$ \\
Exp~3 attack + straggler & $0.9595\pm0.0004$ & $0.9567\pm0.0003$ & $+0.0028$ & $3\times10^{-8}$ \\
Exp~4 honest straggler & $0.9659\pm0.0002$ & $0.9658\pm0.0004$ & $+0.0001$ & $0.25$ \\
\bottomrule
\end{tabular}
\end{table}

Three observations follow. First, the gate provides a large and highly significant benefit under the free-rider attack ($+0.0953$ AUROC, ten of ten seeds), where unclipped random weights would otherwise corrupt the aggregate; the no-detection value ($0.8141$) sits close to the degenerate trimmed-mean result ($0.8242$), consistent with the pipeline reducing to vanilla FedAvg that absorbs the poison once detection is removed. Second, the benefit under gradient scaling and attack-plus-straggler is smaller ($+0.0035$ and $+0.0028$), because a uniformly scaled but directionally-honest update is partly diluted by staleness-weighted aggregation even without detection; the gate still improves on all ten seeds in both settings. Third, and importantly, the gate imposes \emph{no cost} when there is no attacker: on the honest-straggler experiment the gated and ungated results are statistically indistinguishable ($0.9659$ versus $0.9658$, $p=0.25$, and the gated run is higher on only five of ten seeds --- exactly chance), so the detection mechanism does not sacrifice clean-case utility. Together with the entropy result, this shows that SNAS is the only evaluated configuration that both keeps every honest client in the aggregate \emph{and} resists poisoning: Krum resists poisoning only by collapsing to a single client, while no-detection weights all clients but cannot exclude the attacker and so retains the poison.
...
Slow-ramp mimicry is partially detected: the client is first flagged at round 8, never quarantined by round 15, and AUROC degradation remains small. This suggests that flagging and weight penalties can bound damage for gradual attacks, but hard quarantine may require temporal accumulation.

\begin{table}[t]
\caption{Adaptive adversary results. Attack B reveals a robustness boundary of memoryless staleness-normalized scoring.}
\label{tab:adaptive}
\centering
\begin{tabularx}{\textwidth}{@{}lccc>{\raggedright\arraybackslash}X@{}}
\toprule
Attack & Detection & First detection & AUROC $\Delta$ & Interpretation \\
\midrule
Threshold-aware scaling & 100\% & Round 2 & $-0.0054$ & Formula-aware scaling fails due to training drift \\
Alternating, exponential & 0\% & Never & $-0.0057$ & Structural evasion \\
Alternating, linear & 0\% & Never & $-0.0056$ & Structural evasion \\
Alternating, polynomial & 0\% & Never & $-0.0056$ & Structural evasion \\
Alternating, step & 0\% & Never & $-0.0060$ & Worst damage among decay functions \\
Slow ramp, 15 rounds & Flagged & Round 8 & $-0.0013$ & Partial detection bounds damage \\
\bottomrule
\end{tabularx}
\end{table}
...
Table~\ref{tab:sensitivity} summarizes the sensitivity analysis. Detection is stable for $\beta\in[0.10,0.70]$ when $\gamma=1-\beta$, corresponding to 77.8\% of the tested range. Detection degrades only when $\beta\geq0.80$, where the norm-anomaly component is too heavily downweighted. Threshold sweeps over 115 valid $(\delta_{\mathrm{flag}},\delta_{\mathrm{quarantine}})$ combinations show that 71 combinations, or 61.7\%, achieve 100\% detection with 0\% false positives on the hardest evaluated Exp~3 setting. Clip ratio sensitivity from 1.5 to 4.0 shows identical 100\% DR and 0\% FPR across all tested values. Figure~\ref{fig:heatmap} shows the threshold sweep as two panels: detection rate (uniformly $100\%$ across the valid region) and false-positive rate (the quantity that actually varies). The zero-FPR region is a broad plateau rather than a knife-edge, and the default operating point sits comfortably inside it.
...
\midrule
$\beta$ & 0.10--0.70 & 77.8\% of tested values retain 100\% DR \\
...
This study has six main limitations. First, the benchmark uses three clients, reflecting a realistic low-client healthcare federation but limiting conclusions about large-scale cross-device FL. This bound is also what places Krum outside its own $N\geq2f+3$ validity condition, so the Krum comparison in Section~\ref{sec:results} characterizes Krum's behavior in an under-specified regime rather than a fair test of Krum at its intended operating point; a federation of $N\geq5$ is required for that comparison to be conclusive.
